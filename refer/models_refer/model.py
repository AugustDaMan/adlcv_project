...
class VPDRefer(nn.Module):
    ...
    def forward(self, img, l_feats):
        ...
        c_concat = torch.zeros([1,3,512,512]).to(device='cuda')  # Try with dummy hint, this needs to come from the dataloader // August
        #c_concat = None  # Try with no hint // August
        with torch.no_grad():
            latents = self.controlnet.encode_first_stage(img).mode().detach()
        # In controlwrapper, c_concat is interpreted as the hint
        outs_control = self.controlwrapper(latents, t, c_crossattn=[c_crossattn], c_concat=[c_concat])
        
        x_c1, x_c2, x_c3, x_c4 = outs_control  # Replaced by output from ControlNet // August
